...
if (not selected && obj_game.turn == "Defenders")
{
	// Find all of the defender objects
	var _defenders = [];
	for (var _i = 0; _i < instance_number(obj_defender); _i++)
	{
		_defenders[_i] = instance_find(obj_defender, _i);
	}
	// See if any other defenders are selected and exit event if so
	for (var _defender = 0; _defender < array_length(_defenders); _defender++)
	{
		if (_defenders[_defender].selected)
		{
			exit;
		}
	}
	...
	{
		if((Space.Column == 1 or Space.Column == 11) and _space == array_length(_row) - 1)
		{
			break;
		}
